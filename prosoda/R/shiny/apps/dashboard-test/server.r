...
#     
    ...
    widget.titles <- vapply(widget.list.filtered, FUN=function(x){x$title},FUN.VALUE=character(1))
  ...
  observe({
    ...
    if (!is.null(cjson) && isValidJSON(cjson,TRUE)) {
      ...
      for (n in n3) {
        ...
        local({
          nl <- n
          loginfo(paste("Creating output for widget: ",nl))
          tryCatch({
            wout <- widgets.for.rendering[[nl]]
