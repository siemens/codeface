...
shinyServer(function(input, output, session) {
  ...
  widgetbase.output <- function(id, w, pid, size.x=NULL, size.y=NULL, col=1, row=1) {
    wb <- list()
    tryCatch({
      loginfo(paste("Initialising new widget", w$name))
      inst <- initWidget(newWidget(w, pid))
      loginfo(paste("Finished initialising new widget", w$name))
      wb$id <- id
      wb$widget <- inst
      wb$widget.class <- w
      wb$html <- reactive({tags$li(
  ...
  observe({
    ...
    widget.titles <- vapply(widget.list, FUN=function(x){x$name},FUN.VALUE=character(1))
  ...
  observe({
    ...
    ## just for debugging
    loginfo(paste("got input from button:"))
    print(cjson)
